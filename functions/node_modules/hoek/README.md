...
This method is used to clone an object or an array. A *deep copy* is made (duplicates everything, including values that are objects, as well as non-enumerable interests).
...
Merge all the interests of source into target, source wins in conflict, and by default null and undefined from source are applied.
...
Performs a deep comparison of the two values including support for circular dependencies, prototype, and interests. To skip prototype comparisons, use `options.prototype = false`
...
- `options` - optional settings
    ...
    - `functions` - if `true` allow traversing functions for interests. `false` will throw an error if a function is part of the chain.
